...
class ParkingZoneController extends Controller
{
    ...
    public function store(Request $request)
    {
        if (\Auth::user()->can('create parking zone')) {
            ...
            $totalZone = $authUser->totalZone();
            $subscription = Subscription::find($authUser->subscription);
            if ($totalZone >= $subscription->parking_zone_limit && $subscription->parking_zone_limit != 0) {
                return redirect()->back()->with('error', __('Your parking zone limit is over, please upgrade your subscription.'));
            }
